...
function createUIForLocalPlayer(self) {
  ...
  self.player1ScoreText = self.add
    .text(20, 20, "Score: 0", {
      font: "24px Digital-7",
      fill: "#0ff",
    })
    .setShadow(2, 2, "#00f", 2);
  let player1Health = createHealthBar(self, 20, 50);
  self.player1HealthBar = player1Health.blocks;
}

function createUIForOtherPlayer(self) {
  // Player 2 UI (Top-right)
  self.player2ScoreText = self.add
    .text(self.sys.game.config.width - 150, 20, "Score: 0", {
      font: "24px Digital-7",
      fill: "#f00",
    })
    .setShadow(2, 2, "#f00", 2);

  let player2Health = createHealthBar(self, 900, 50, true);
...
function createHealthBar(self, x, y, isRightAligned = false) {
  ...
  let blockWidth = 12; // Increased for better visibility
  let gap = 3;
  let totalBlocks = 10; // Total health blocks
  let blocks = [];

  let totalWidth = totalBlocks * (blockWidth + gap) - gap;

  // Add "Health" text label centered above the bar
  let healthText = self.add
    .text(0, -20, "Health", {
      fontSize: "16px",
      fill: "#ffffff",
      fontFamily: "Arial",
  ...
  for (let i = 0; i < totalBlocks; i++) {
    let blockX = (blockWidth + gap) * i;
    if (isRightAligned) {
      blockX = -(blockWidth + gap) * i; // Shift left for right-aligned bars
    }
  ...
  // Adjust position for right-aligned bars
  if (isRightAligned) {
    healthBarContainer.setX(x - (blockWidth + gap) * (totalBlocks - 1));
  }
...
function updateScore(self, player, player_id) {
  let scoreText =
    player_id === self.localPlayerId
      ? self.player1ScoreText
      : self.player2ScoreText;
